...
public class SyncCall implements Callable<Boolean>
{
   ...
   public Boolean call()
   {
      if ("save".equals(action))
      {
         ...
         if(!MineBackup.saving) // prevent re-entrance while still running
         {
            if(plugin.getConfig().contains("worldsToSaveWithoutPlayers"))
            {
               ...
               {
                  if(worldsToSaveWithoutPlayers.contains(world.getName()))
                  {
                     MineBackup.saving = true;

                     if(Debug.ON){Debug.info(MineBackup.logPrefix + "Kicking all players still online...");}
                     for(Player p : Bukkit.getServer().getOnlinePlayers())
                     {
                        p.kickPlayer(MineBackup.worldSaveMsg);
                     }
                  }
               }
            ...
            if(MineBackup.saving && Debug.ON)
            {
               duration = System.currentTimeMillis() - duration;
               Debug.info(MineBackup.logPrefix + "-> Success. Saving took " + duration + " ms.");
            }

            MineBackup.saving = false;
         }
         else
         {
            world.save();
